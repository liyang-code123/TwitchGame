...
//                    break outerLoop;
    ...
    private List<Item> recommendByFavoriteHistory(Set<String> favoriteItemIds, List<String> favoriteGameIds, ItemType type)
            throws RecommendationException {

        // Count the favorite game IDs from the database for the given user.
        // E.g. if the favorite game ID list is ["1234", "2345", "2345", "3456"],
        // then returned Map is {"1234": 1, "2345": 2, "3456": 1}

        // Java 8 stream and it can run parallel.
        Map<String, Long> favoriteGameIdByCount = favoriteGameIds.parallelStream().collect(Collectors.groupingBy(Function.identity(),
                Collectors.counting()));
        /*
        ...
        for(String id : favoriteGameIds) {
            favoriteGameIdByCount.put(id, favoriteGameIdByCount.getOrDefault(id, 0) + 1);

        }

         */

        // Sort the game Id by count. E.g.
        // if the input is {"1234": 1, "2345": 2, "3456": 1},
        // then returned Map is {"2345": 2, "1234": 1, "3456": 1}
        ...
        if (sortedFavoriteGameIdListByCount.size() > DEFAULT_GAME_LIMIT) {
            sortedFavoriteGameIdListByCount.subList(0, DEFAULT_GAME_LIMIT);
        }
        ...
        outerLoop:
        for (Map.Entry<String, Long> favroiteGame : sortedFavoriteGameIdListByCount) {
            List<Item> items;
            try {
                items = client.searchByType(favroiteGame.getKey(), type, DEFAULT_PER_GAME_RECOMMENDATION_LIMIT);
            } catch (TwitchException e) {
                e.printStackTrace();
